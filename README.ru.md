...
Стадии: рисёрч ниши/конкурентов (через твой **NexLev / vidIQ** MCP, если подключён, иначе web-поиск) → скрипт закадра → картинка каждые 5–7с и **анимация первых кадров** (полностью статичные AI-видео YouTube приглушает) → озвучка ElevenLabs → сборочный кит + графика-оверлеи под CapCut, либо авто-сборка через hyperframes, если пользуешься.
...
| `KIE_WORKSPACE_DIR` | *(не задана — путь от рабочей папки)* | Рекомендуется |

Задаются в блоке `env` твоего MCP-клиента (см. конфиг выше). `KIE_WORKSPACE_DIR` = папка проекта — тогда все чтение/запись файлов заперты в ней.
...
- 📁 С заданным `KIE_WORKSPACE_DIR` аплоад/даунлоад заперты в этой папке (откажет от `/` или голого домашнего каталога).
- 🖼️ Аплоад — только известные медиа-типы; даунлоад отказывается писать dotfiles, скрипты и исполняемые.
...
```bash
# обновить
cd kie-mcp-kit && git pull            # затем перезапусти ./install.sh, если менялись скиллы
