...
> **Работает в любом MCP-клиенте, не только в Claude Code.** Скиллы вшиты и в коннектор:
> в обычном чате Claude Desktop, на claude.ai или в Cursor — где файловые скиллы не грузятся —
> агент вызывает `kie_workflows`, подтягивает те же инструкции и гонит пайплайн. В Claude Code
> файловые скиллы триггерятся сами; в остальных местах их несёт тул.
...
| `kie_workflows(workflow?)` | Список воркфлоу или загрузка полной инструкции одного. Работает в **любом** MCP-клиенте — файловый скилл не нужен. |
| `kie_workflow_file(workflow, path)` | Прочитать reference-файл воркфлоу (напр. библиотеку промптов). |
...
- 📁 **Даунлоад** (запись) заперт в workspace (`KIE_WORKSPACE_DIR`, иначе рабочая папка), симлинки учитываются; `/` и голый домашний каталог отвергаются. **Аплоад** (чтение) — из любой папки внутри домашнего каталога или workspace: чтение менее рискованно, а медиа-allowlist и так режет секреты.
...
| `kie_download` вернул `ok:false` с hint | Хост результата был медленным/недоступным; теперь падает за ~45с, а не висит. Повтори или открой `url` напрямую (ссылки живут ~24ч). |
